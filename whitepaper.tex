...
Harvey's published LAB results include per-model breakdowns showing that Gemini models — the same model family used here — achieved \textbf{0\% strict all-pass} when used in a conventional single-pass pipeline. The same models that produce zero successful tasks individually achieve 17.75\% strict all-pass when coordinated through a stateful swarm.
